...
import { TaskTimeTrackerSettings } from "./settings";
...
export function displayTracker(tracker: Tracker, element: HTMLElement, getSectionInfo: () => MarkdownSectionInformation, settings: TaskTimeTrackerSettings): void {
    ...
    let btn = new ButtonComponent(element)
        ...
        .onClick(async () => {
            if (running) {
                ...
                startNewEntry(tracker, newTaskNameBox.getValue());
            }
            await saveTracker(tracker, this.app, getSectionInfo());
        });
    btn.buttonEl.addClass("task-time-tracker-btn");

    let newTaskNameBox = new TextComponent(element)
        .setPlaceholder("Task name")
        .setDisabled(running);
    newTaskNameBox.inputEl.addClass("task-time-tracker-name-txt");

    // add timers
    let timer = element.createDiv({ cls: "task-time-tracker-timers" });
    let currentDiv = timer.createEl("div", { cls: "task-time-tracker-timer" });
    let current = currentDiv.createEl("span", { cls: "task-time-tracker-timer-time" });
    currentDiv.createEl("span", { text: "Current" });
    let totalDiv = timer.createEl("div", { cls: "task-time-tracker-timer" });
    let total = totalDiv.createEl("span", { cls: "task-time-tracker-timer-time", text: "0s" });
    totalDiv.createEl("span", { text: "Total" });

    if (tracker.entries.length > 0) {
        // add table
        let table = element.createEl("table", { cls: "task-time-tracker-table" });
        table.createEl("tr").append(
            createEl("th", { text: "Tasks" }),
        ...
        //add TextComponent
        let startBox = new TextComponent(element)
            .setPlaceholder("Start Time")
            .setDisabled(running);;
        startBox.inputEl.addClass("task-time-tracker-start-txt");

        let endBox = new TextComponent(element)
            .setPlaceholder("End Time")
            .setDisabled(running);;
        endBox.inputEl.addClass("task-time-tracker-end-txt");

        // add copy buttons
        let buttons = element.createEl("div", { cls: "task-time-tracker-bottom" });
        ...
        new ButtonComponent(buttons)
            ...
            .onClick(async () => {
                if (!running) {
                    addTime(tracker, newTaskNameBox.getValue(), startBox.getValue(), endBox.getValue())
                }
                await saveTracker(tracker, this.app, getSectionInfo());
            });

        new ButtonComponent(buttons)
            .setButtonText("Add sub time")
            .onClick(async () => {
                if (!running) {
                    addSubTime(tracker, newTaskNameBox.getValue(), startBox.getValue(), endBox.getValue())
                }
                await saveTracker(tracker, this.app, getSectionInfo());
            });
...
function formatTimestamp(timestamp: number, settings: TaskTimeTrackerSettings): string {
...
function createMarkdownTable(tracker: Tracker, settings: TaskTimeTrackerSettings): string {
    let table = [["Tasks", "Start time", "End time", "Duration"]];
    for (let entry of tracker.entries)
...
function createCsv(tracker: Tracker, settings: TaskTimeTrackerSettings): string {
    let ret = "";
    for (let entry of tracker.entries) {
...
function createTableSection(entry: Entry, settings: TaskTimeTrackerSettings): string[][] {
    let ret: string[][] = [[
        entry.name,
...
function addEditableTableRow(tracker: Tracker, entry: Entry, table: HTMLTableElement, newTaskNameBox: TextComponent, running: boolean, getSectionInfo: () => MarkdownSectionInformation, settings: TaskTimeTrackerSettings, indent: number) {
...
function addSubTime(tracker: Tracker, taskName: string, start: string, end: string) {
    // let entry: Entry = { name: null, startTime: null,endTime: null};
    for (let i = 0; i < tracker.entries.length; i++) {
        if (taskName == tracker.entries[i].name) {
            if (!tracker.entries[i].subEntries) {
                tracker.entries[i].subEntries = [{ ...tracker.entries[i], name: `Part 1` }];
                tracker.entries[i].startTime = null;
                tracker.entries[i].endTime = null;
            }

            let date: string = String(moment().format("YYYY-MM-DD"));
            start = date + " " + start + ":00";
            end = date + " " + end + ":00";
            let sTime = new Date(start.replace(/-/g, '/')).getTime();
            let eTime = new Date(end.replace(/-/g, '/')).getTime();

            let name = `Part ${tracker.entries[i].subEntries.length + 1}`;
            let entry: Entry = { name: name, startTime: sTime / 1000, endTime: eTime / 1000, subEntries: null };
            tracker.entries[i].subEntries.push(entry);
        }
    }
}

function addTime(tracker: Tracker, taskName: string, start: string, end: string): void {
    let date: string = String(moment().format("YYYY-MM-DD"));
    start = date + " " + start + ":00";
    end = date + " " + end + ":00";
    let sTime = new Date(start.replace(/-/g, '/')).getTime();
    let eTime = new Date(end.replace(/-/g, '/')).getTime();
    // console.log(String(eTime))
    if (!taskName)
        taskName = `Task ${tracker.entries.length + 1}`;
    let entry: Entry = { name: taskName, startTime: sTime / 1000, endTime: eTime / 1000, subEntries: null };
    tracker.entries.push(entry);
}
